...
export default function CaseDetailScreen() {
  ...
  const wideDesktop = width >= 1180;
  const compact = width < 600;
  const pageContentWidth = Math.max(280, Math.min(width, 1120) - 40);
  const galleryWidth = wideDesktop
    ? Math.max(420, pageContentWidth - 340 - 38)
    : pageContentWidth;
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><PublicHeader /><KeyboardAwareScrollView contentContainerStyle={styles.page}>
    ...
    <View style={[styles.layout, wideDesktop && styles.layoutDesktop]}>
      <View style={styles.main}>
        <View style={[styles.gallery, { width: galleryWidth }]}>
          <ScrollView ref={galleryRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false} onMomentumScrollEnd={(event) => setActiveImage(Math.round(event.nativeEvent.contentOffset.x / galleryWidth))}>
            {images.map((entry) => <Pressable key={entry.id} onPress={() => setViewerOpen(true)} style={{ width: galleryWidth }}><Image source={entry.url} style={[styles.heroImage, !wideDesktop && styles.heroImageStacked, compact && styles.heroImageMobile]} contentFit="cover" transition={250} alt={entry.altText || `Hình ảnh của ${item.name}`} /></Pressable>)}
          </ScrollView>
        ...
        {!!item.video && <View style={styles.caseVideoSection}><Text style={styles.caseVideoTitle}>{item.video.title || 'Video hoàn cảnh'}</Text><CaseVideoPlayer video={item.video} /></View>}
      ...
      <View style={[styles.side, wideDesktop && styles.sideDesktop]}>
